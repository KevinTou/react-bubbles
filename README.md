...
- [ ] Explain what a token is used for.
  ...
  > A token is used to notify the server that whoever makes a request to a protected endpoint, using a valid token, is authorized to do so.

- [ ] What steps can you take in your web apps to keep your data secure?

  > You can create protected routes, which will not allow a user to reach there unless there is a JWT present.

- [ ] Describe how web servers work.

  > When a user makes a request, it is sent to their local internet access provider. That request is passed around until it reaches the correct server. A response is then sent back and eventually translated back down into code to display data to a user.
